...
| 種類             | 概要                                                         | 設定箇所                     | コード                   |
| ---------------- | ------------------------------------------------------------ | ---------------------------- | ------------------------ |
| グローバル変数   | ワークスペース全体で有効な変数。                             | Environments → Globals       | `pm.globals`             |
| コレクション変数 | コレクションのリクエスト内で有効な変数。<br />（環境に関わらず不変） | Collections → 各コレクション | `pm.collectionVariables` |
| 環境変数         | 環境に応じて変化する変数。                                   | Environments → 各環境        | `pm.environment`         |
| データ変数       | 外部のCSV, JSONファイルをもとに定義される変数。              |                              |                          |
| ローカル変数     | リクエストスクリプト内で一時的に有効な変数。                 |                              | `pm.variables`           |
