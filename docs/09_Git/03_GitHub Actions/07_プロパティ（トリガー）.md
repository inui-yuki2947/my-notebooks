...
| 分類       | プロパティ                       | 型             | 概要                                                         |
| ---------- | -------------------------------- | -------------- | ------------------------------------------------------------ |
| 基本       | `on.push`                        |                | コミットまたはタグがプッシュされたタイミング。               |
| 基本       | `on.pull_request`                |                | プルリクエストが、開かれた・再度開かれた・そのHEADが更新されたタイミング。 |
| 基本       | `on.pull_request_review`         |                | プルリクエストのレビューが、送信・編集・削除されたタイミング。 |
| フィルター | `on.イベント.branches` ※1        | 文字列の配列型 | イベントを適用するブランチ名。                               |
| フィルター | `on.イベント.branches-ignore`※1 | 文字列の配列型 | イベントを適用除外するブランチ名。                           |
| フィルター | `on.イベント.tags`※1            | 文字列の配列型 | イベントを適用するタグ名。                                   |
| フィルター | `on.イベント.tags-ignore` ※1     | 文字列の配列型 | イベントを適用除外するタグ名。                               |
| フィルター | `on.イベント.paths`※1           | 文字列の配列型 | イベントを適用するパス名。                                   |
| フィルター | `on.イベント.paths-ignore`※1    | 文字列の配列型 | イベントを適用除外するパス名。                               |

- ※1 当プロパティは、`push`, `pull_request`, `pull_request_target`イベントでのみ使用可能。
