...
- IPv6環境では、`ping6`, `traceroute6`, `tracepath6`コマンドを使う。
...
- `traceroute|tracepath`コマンドは、パケットが伝わる経路を表示する。

  ```bash
  traceroute|tracepath ホスト名またはIPアドレス
  ```

### route

- `route`コマンドは、ルーティングテーブルを表示する。
...
- `route add|del パラメータ`コマンドは、ルーティングテーブルに新たな経路を追加・削除する。
...
- `netstat`コマンドは、ネットワークに関する様々な情報を表示する。
...
- `host`コマンドは、DNSのサーバを使って、ホストやドメインに関する情報を表示する。

  ```bash
  host ホスト名またはIPアドレス [DNSサーバ]
  ```

### nslookup

- `nslookup`コマンドは、ドメイン名からIPアドレスを取得する。
...
- `dig`コマンドは、ドメイン名からIPアドレスを取得する。

  ```bash
  dig [@DNSサーバ] ホスト名またはドメイン名 [クエリタイプ] [クエリクラス] [クエリオプション]
  ```
