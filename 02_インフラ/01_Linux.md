...
- `/usr/local`は、更に`bin, sbin lib`などのディレクトリに細分化され、ローカルシステムで必要とされるコマンドが配置される。
...
- 「ext4」は、多くのLinuxディストリビューションの標準ファイルシステムである。
- 「Btrfs（B-tree file system）」は、Linux向けの新しいファイルシステムである。
- 「XFS」は、CentOS 7などで標準となっている、ジャーナリングファイルシステムである。

### mkfs, mke2fs, mkfs.btrfs, mkfs.xfs

- `mkfs`コマンドは、パーティション上に各種のファイルシステムを作成する。

  ```bash
  mkfs デバイス名
  ```

- `mke2fs`コマンドは、ext2, ext3, ext4のファイルシステムを作成する。

  ```bash
  mke2fs デバイス名
  ```

- `mkfs.btrfs`コマンドは、Btrfsのファイルシステムを作成する。

  ```bash
  mkfs.btrfs デバイス名
  ```

- `mkfs.xfs`コマンドは、XFSのファイルシステムを作成する。
...
- `unmount`コマンドは、ファイルシステムをアンマウントする。

  ```bash
  unmount デバイス名またはマウントポイント
  ```
