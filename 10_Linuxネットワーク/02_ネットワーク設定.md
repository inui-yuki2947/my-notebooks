...
- `ifconfig [インターフェース名]`コマンドは、ネットワークインターフェースの情報を表示する。

  ```bash
  ifconfig [インターフェース名]
  ```

### ifconfig  インターフェース名 操作

- `ifconfig インターフェース名 操作`コマンドは、ネットワークインターフェースの操作を行う。
...
- 操作
  - `up`は、インタフェースを有効にする。
  - `down`は、インタフェースを無効にする。
  - `IPアドレス`は、IPアドレスを設定する。
  - `netmask サブネットマスク`は、サブネットマスクを設定する。
...
- `nmcli`コマンドは、Red Hat系ディストリビューションで、NetworkManagerの設定を行う。
...
- `-l`オプションで、指定したポートをリッスンする。
- `-o 出力ファイル名`オプションで、指定したファイルに出力する。
